...
def prepare_data(char_seqs, labels, maxw, maxwlen, dim_proj):
    ...
    x_c = numpy.zeros((int(maxw), int(maxwlen), int(n_samples))).astype('int8')
    x_mask = numpy.zeros((int(maxw), int(maxwlen), int(n_samples), int(dim_proj))).astype(theano.config.floatX)
    y = numpy.zeros(int(n_samples)).astype('int8')
    y_mask = numpy.zeros(int(n_samples)).astype('float32')
        ...
        for c, warr in enumerate(s_c):
            ...
            for p, carr in enumerate(warr): # huh! good God!
                ...
                if p >= maxwlen:
                    ...
                    break
                x_c[c, p, idx] = carr
                x_mask[c, p, idx] = numpy.ones(int(dim_proj))
