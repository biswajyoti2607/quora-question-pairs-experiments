...
def main():
	...
	if mode == "train":
		...
		test_data_gen, test_data_size = data_manager.get_test_data_from_file([config.test_file])
		num_test_steps = int(math.ceil(test_data_size / batch_size))
		
		raw_predictions = model.train(get_train_instance_generator=get_train_data_gen,
		...
		#test_batch_gen = DataManager.get_batch_generator(test_data_gen, batch_size)
		#y_pred_model = None
		#y_pred = []
		# sentence_one = tf.placeholder("int32",
										   # [None, None],
										   # name="sentence_one")
		# sentence_two = tf.placeholder("int32",
										   # [None, None],
										   # name="sentence_two")
		# y_true = tf.placeholder("int32",
									 # [None, 2],
									 # name="true_labels")
		...
		# for batch in tqdm(test_batch_gen,
			...
			# inputs, _ = batch
			# feed_dict = {sentence_one: inputs[0],
					 # sentence_two: inputs[1],
					 # is_train: False}
			# y_pred_batch = sess.run(y_pred_model, feed_dict=feed_dict)
			# y_pred.append(y_pred_batch)
		#y_pred_flat = np.concatenate(y_pred, axis=0)
		#raw_predictions = y_pred_flat
